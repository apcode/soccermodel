...
class SoccerRatingModelPyTorch:
    ...
    def create_datasets(self, df, val_split=0.1, batch_size=512):
        """
        ...
        # Convert to tensors
        home_teams = torch.tensor(
            df["home_team_encoded"].values, dtype=torch.long, device=self.device
        )
        away_teams = torch.tensor(
            df["away_team_encoded"].values, dtype=torch.long, device=self.device
        )
        home_goals = torch.tensor(
            df["home_team_goal"].values, dtype=torch.float32, device=self.device
        )
        away_goals = torch.tensor(
            df["away_team_goal"].values, dtype=torch.float32, device=self.device
        )
        ...
        train_dataset = TensorDataset(
            train_home_teams, train_away_teams, train_home_goals, train_away_goals
        )
        val_dataset = TensorDataset(
            val_home_teams, val_away_teams, val_home_goals, val_away_goals
        )
    ...
    def fit(
        self, df, epochs=1000, lr=0.01, weight_decay=1e-4, batch_size=512, patience=50
    ):
        """
        ...
        print(
            f"Training for up to {epochs} epochs with early stopping (patience={patience})"
        )
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0 or epoch < 20:
                ...
                print(
                    f"{epoch:5d} | {avg_train_loss:10.4f} | {avg_val_loss:8.4f} | {current_lr:.2e} | {home_adv:6.3f}"
                )
        ...
        print(
            f"Attack ratings range: [{self.attack_ratings.min():.3f}, {self.attack_ratings.max():.3f}]"
        )
        print(
            f"Defense ratings range: [{self.defense_ratings.min():.3f}, {self.defense_ratings.max():.3f}]"
        )
        ...
        with torch.no_grad():
            for i in range(self.n_teams):
                ...
                if total_matches > 0:
                    ...
                    self.model.attack_ratings[i] = torch.log(
                        torch.tensor(max(avg_scored, 0.1))
                    )
                    ...
                    self.model.defense_ratings[i] = -torch.log(
                        torch.tensor(max(avg_conceded, 0.1))
                    )
    ...
    def get_team_ratings(self, df):
        ...
        team_names = {}
        for _, row in (
            df[["home_team_api_id", "home_team_name"]].drop_duplicates().iterrows()
        ):
            team_names[row["home_team_api_id"]] = row["home_team_name"]
        for _, row in (
            df[["away_team_api_id", "away_team_name"]].drop_duplicates().iterrows()
        ):
        ...
        ratings_df["overall_strength"] = (
            ratings_df["attack_rating"] - ratings_df["defense_rating"]
        )
    ...
    def evaluate_model(self, df_test, batch_size=512):
        """
        ...
        home_teams = torch.tensor(
            df_test["home_team_encoded"].values, dtype=torch.long, device=self.device
        )
        away_teams = torch.tensor(
            df_test["away_team_encoded"].values, dtype=torch.long, device=self.device
        )
        home_goals = torch.tensor(
            df_test["home_team_goal"].values, dtype=torch.float32, device=self.device
        )
        away_goals = torch.tensor(
            df_test["away_team_goal"].values, dtype=torch.float32, device=self.device
        )

        # Create test dataset
        test_dataset = TensorDataset(home_teams, away_teams, home_goals, away_goals)
        ...
        test_loss = 0.0
        test_batches = 0
        all_predictions_home = []
        all_predictions_away = []
        all_actuals_home = []
        all_actuals_away = []
        ...
        with torch.no_grad():
            for (
                ...
                all_predictions_home.extend(lambda_home.cpu().numpy())
                all_predictions_away.extend(lambda_away.cpu().numpy())
                all_actuals_home.extend(batch_home_goals.cpu().numpy())
                all_actuals_away.extend(batch_away_goals.cpu().numpy())

        # Convert to numpy arrays
        pred_home = np.array(all_predictions_home)
        pred_away = np.array(all_predictions_away)
        actual_home = np.array(all_actuals_home)
        actual_away = np.array(all_actuals_away)
        ...
        rmse_home = np.sqrt(np.mean((pred_home - actual_home) ** 2))
        rmse_away = np.sqrt(np.mean((pred_away - actual_away) ** 2))

        # Match outcome accuracy
        pred_home_win = pred_home > pred_away
        pred_draw = np.abs(pred_home - pred_away) < 0.1  # Small threshold for draws
        pred_away_win = pred_away > pred_home

        actual_home_win = actual_home > actual_away
        actual_draw = actual_home == actual_away
        actual_away_win = actual_away > actual_home

        # Outcome prediction accuracy (simplified - most likely outcome)
        pred_outcomes = np.where(
            pred_home > pred_away, "H", np.where(pred_away > pred_home, "A", "D")
        )
        actual_outcomes = np.where(
            actual_home > actual_away,
            "H",
            np.where(actual_away > actual_home, "A", "D"),
        )
        outcome_accuracy = np.mean(pred_outcomes == actual_outcomes)

        # Print results
        print(f"\nTest Results:")
        print(f"  Test Loss: {avg_test_loss:.4f}")
        print(f"\nGoal Prediction Accuracy:")
        print(f"  Home Goals - MAE: {mae_home:.3f}, RMSE: {rmse_home:.3f}")
        print(f"  Away Goals - MAE: {mae_away:.3f}, RMSE: {rmse_away:.3f}")
        print(f"  Overall Goal MAE: {(mae_home + mae_away)/2:.3f}")

        print(f"\nMatch Outcome Prediction:")
        print(f"  Outcome Accuracy: {outcome_accuracy:.1%}")

        # Detailed outcome breakdown
        home_win_count = np.sum(actual_home_win)
        draw_count = np.sum(actual_draw)
        away_win_count = np.sum(actual_away_win)

        print(f"\nActual Results Distribution:")
        print(f"  Home Wins: {home_win_count} ({home_win_count/len(actual_home):.1%})")
        print(f"  Draws: {draw_count} ({draw_count/len(actual_home):.1%})")
        print(f"  Away Wins: {away_win_count} ({away_win_count/len(actual_home):.1%})")

        # Expected vs actual goal averages
        print(f"\nGoal Averages:")
        print(f"  Expected: Home {pred_home.mean():.2f}, Away {pred_away.mean():.2f}")
        print(
            f"  Actual:   Home {actual_home.mean():.2f}, Away {actual_away.mean():.2f}"
        )
        ...
        pred_over_25 = pred_total > 2.5
        actual_over_25 = actual_total > 2.5
        over_under_accuracy = np.mean(pred_over_25 == actual_over_25)

        print(f"\nOver/Under 2.5 Goals:")
        print(f"  Prediction Accuracy: {over_under_accuracy:.1%}")
        print(f"  Expected O2.5 Rate: {np.mean(pred_over_25):.1%}")
        print(f"  Actual O2.5 Rate: {np.mean(actual_over_25):.1%}")

        # Log-likelihood per match
        test_ll_per_match = -avg_test_loss
        print(f"\nModel Fit:")
        print(f"  Log-likelihood per match: {test_ll_per_match:.3f}")
        ...
        return {
            ...
            "log_likelihood_per_match": test_ll_per_match,
            "pred_home_avg": pred_home.mean(),
            "pred_away_avg": pred_away.mean(),
            "actual_home_avg": actual_home.mean(),
            "actual_away_avg": actual_away.mean(),
        }

    def plot_predictions_vs_actual(self, df_test, save_path=None, n_samples=1000):
        """
        Plot predicted vs actual goals to visualize model performance
        """
        if not self.is_fitted:
        ...
        if len(df_test) > n_samples:
            df_sample = df_test.sample(n_samples, random_state=42)
        else:
            df_sample = df_test
        ...
        print(f"Generating predictions for {len(df_sample)} matches...")
        ...
        for _, match in df_sample.iterrows():
            try:
                ...
                actuals.append([match["home_team_goal"], match["away_team_goal"]])
            except:
                continue
        ...
        # Home goals
        axes[0].scatter(predictions[:, 0], actuals[:, 0], alpha=0.6, s=20)
        axes[0].plot([0, 5], [0, 5], "r--", alpha=0.8, label="Perfect Prediction")
        ...
        # Away goals
        axes[1].scatter(predictions[:, 1], actuals[:, 1], alpha=0.6, s=20)
        axes[1].plot([0, 5], [0, 5], "r--", alpha=0.8, label="Perfect Prediction")
...
def main():
    ...
    print(f"\nData split:")
    print(
        f"Training: {len(df_train):,} matches ({df_train['date'].min()} to {df_train['date'].max()})"
    )
    print(
        f"Testing:  {len(df_test):,} matches ({df_test['date'].min()} to {df_test['date'].max()})"
    )
    ...
    print(
        ratings.head(10)[
            ["team_name", "attack_rating", "defense_rating", "overall_strength"]
        ]
    )

    print("\n" + "=" * 40)
    print("BOTTOM 5 TEAMS BY OVERALL STRENGTH")
    print("=" * 40)
    print(
        ratings.tail(5)[
            ["team_name", "attack_rating", "defense_rating", "overall_strength"]
    ...
    test_metrics = model.evaluate_model(df_test)

    # Create visualizations
    print("\nGenerating evaluation plots...")
    model.plot_predictions_vs_actual(
        df_test, save_path="models/pytorch_predictions_vs_actual.png"
    )
    ...
    print("=" * 60)
    print(f"Training Data: {len(df_train):,} matches")
    print(f"Test Data: {len(df_test):,} matches")
    print(f"Total Teams: {model.n_teams}")
    print(f"Home Advantage: {model.home_advantage:.3f}")
    print(
        f"Test MAE (goals): {(test_metrics['mae_home'] + test_metrics['mae_away'])/2:.3f}"
    )
    print(f"Test Outcome Accuracy: {test_metrics['outcome_accuracy']:.1%}")
    print(f"Over/Under 2.5 Accuracy: {test_metrics['over_under_accuracy']:.1%}")
