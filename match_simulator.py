#!/usr/bin/env python3
"""
Match Simulation Engine for Soccer Predictions

This script uses the trained Poisson model to simulate match outcomes and provide:
- Match result probabilities (Home Win, Draw, Away Win)
- Detailed scoreline probabilities up to 4-4
- Most likely scorelines
- Expected goals and confidence intervals
"""
...
class MatchSimulator:
    """
    Soccer match simulator using Poisson goal model
    """
    
    def __init__(self, model_path="models/soccer_rating_model.pkl"):
        """Initialize simulator with trained model"""
        self.model = SoccerRatingModel()
        try:
            self.model.load_model(model_path)
            print(f"Model loaded successfully from {model_path}")
        except FileNotFoundError:
            print(f"Model not found at {model_path}")
            print("Please run team_rating_model.py first to train a model")
            raise
    ...
    def simulate_match(self, home_team, away_team, max_goals=5, num_simulations=10000):
        ...
        Parameters:
        - home_team: Team name or API ID
        - away_team: Team name or API ID  
        - max_goals: Maximum goals to calculate probabilities for
        - num_simulations: Number of Monte Carlo simulations
        
        Returns:
        - Dictionary with comprehensive match predictions
        """
        # Get team IDs
    ...
    def create_probability_heatmap(self, prediction, save_path=None):
        """Create a heatmap visualization of scoreline probabilities"""
        ...
        # Create heatmap
        sns.heatmap(
            prob_matrix * 100,  # Convert to percentages
            annot=True,
            fmt='.1f',
            cmap='Blues',
        ...
        plt.title(f'Scoreline Probabilities: {prediction["home_team"]} vs {prediction["away_team"]}')
        plt.xlabel(f'{prediction["away_team"]} Goals')
        plt.ylabel(f'{prediction["home_team"]} Goals')
        
        # Add expected goals to title
        home_exp = prediction['expected_goals']['home']
        away_exp = prediction['expected_goals']['away']
        plt.suptitle(f'Expected Goals: {home_exp:.2f} - {away_exp:.2f}', y=0.95)
    ...
    def create_results_chart(self, prediction, save_path=None):
        """Create a bar chart of match result probabilities"""
        results = prediction['match_results']
        
        plt.figure(figsize=(10, 6))
        
        # Create bar chart
        categories = [prediction['home_team'], 'Draw', prediction['away_team']]
        probabilities = [results['home_win'], results['draw'], results['away_win']]
        ...
        plt.title(f'Match Result Probabilities: {prediction["home_team"]} vs {prediction["away_team"]}')
        plt.ylabel('Probability')
        plt.ylim(0, max(probabilities) * 1.2)
        
        # Add expected goals as subtitle
        home_exp = prediction['expected_goals']['home']
        away_exp = prediction['expected_goals']['away']
        plt.suptitle(f'Expected Goals: {home_exp:.2f} - {away_exp:.2f}', y=0.95)
...
def main():
    """Main function for interactive match simulation"""
    print("Soccer Match Simulator")
    print("=" * 40)
    ...
    while True:
        try:
            ...
            if viz == 'y':
                ...
                simulator.create_probability_heatmap(prediction)
                
                print("Creating results chart...")
                simulator.create_results_chart(prediction)
